    ...
    <div class="min-h-screen flex">
        ...
        <aside class="w-64 bg-slate-900 text-slate-200 flex flex-col shrink-0">
            <div class="px-6 py-5 border-b border-slate-800">
                <p class="text-white font-semibold leading-tight">{{ config('app.name') }}</p>
            ...
            <nav class="flex-1 px-3 py-4 space-y-1">
                ...
                <a href="{{ route('kontak.index') }}"
                   class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition
                          {{ request()->routeIs('kontak.*') ? 'bg-slate-800 text-white' : 'text-slate-300 hover:bg-slate-800 hover:text-white' }}">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
        ...
        <div class="flex-1 flex flex-col min-w-0">
            <header class="bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div class="text-sm">
                        <p class="font-medium leading-tight">{{ auth()->user()->name ?? 'Pengguna' }}</p>
